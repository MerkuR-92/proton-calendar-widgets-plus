...
internal class ObtainSendPreferencesUseCaseTest {
    ...
    fun `before each`() {
        ...
        coEvery { contactEmailsRepositoryMock.getContactEmails(userId, refresh = true) } returns listOf(
            ContactEmail("1", "External Contact with pinned key", "contact_external_pinned_key+alias@email.com", defaults = 0, 1, contactId = "contact_1", "contact_external_pinned_key@email.com")
        )

        coEvery { contactEmailsRepositoryMock.getContact(userId, "contact_1", refresh = true) } returns externalContactWithPinnedKey
    ...
    fun `handle createDefaultSendPreferences for unknown external recipient with keys`() {
        ...
        val result = sut.createDefaultSendPreferences(mailSettingsSignFalse.data.mailSettings.toMailSettings()!!, unknownExternalRecipientWithKeysPublicAddress)!!
    ...
    fun `external contact with pinned key and email alias should be fetched, VCard extracted`() {
        runBlocking {
            ...
            coVerify(exactly = 1) { externalContactWithPinnedKey.extractSignedVCard(userMock, cryptoContextMock, logger) }
    ...
    private val mailSettingsSignTrue = ApiResponse.Success<MailSettingsApiResponse>(json.decodeFromString(
        """
    ...
    private val unknownExternalRecipientWithKeysPublicAddress = PublicAddress(
        ...
        "text/html",
        listOf(
            PublicAddressKey("unknown_external_with_keys@email.com", 0, PublicKey("armored key", isPrimary = false)),
            PublicAddressKey("unknown_external_with_keys@email.com", 0, PublicKey("armored key", isPrimary = true))
        )
    )
    ...
    private val unknownInternalRecipientWithKeysPublicAddress = PublicAddress(
        ...
        "text/html",
        listOf(
            PublicAddressKey("unknown_internal@pm.me", 0, PublicKey("armored key", isPrimary = false)),
            PublicAddressKey("unknown_internal@pm.me", 0, PublicKey("armored key", isPrimary = true))
        )
    )
    ...
    private val externalContactWithPinnedKey: Contact =
        Contact(
