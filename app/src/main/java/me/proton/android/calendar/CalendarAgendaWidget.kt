...
class CalendarAgendaWidget : AppWidgetProvider(), KoinComponent {
...
internal class CalendarAgendaWidgetService : RemoteViewsService(), KoinComponent {
    override fun onGetViewFactory(intent: Intent?): RemoteViewsFactory {
        val widgetId = intent?.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, AppWidgetManager.INVALID_APPWIDGET_ID)
            ?: AppWidgetManager.INVALID_APPWIDGET_ID
        return CalendarAgendaWidgetFactory(widgetId)
    }
}

internal class CalendarAgendaWidgetFactory(
    ...
    override fun onDataSetChanged() {
        if (appWidgetId == AppWidgetManager.INVALID_APPWIDGET_ID) {
            logger.e("Legacy Widget factory got INVALID_APPWIDGET_ID")
            data = emptyList()
            return
        }
    ...
    override fun getViewAt(position: Int): RemoteViews? {
        val event = data.getOrNull(position) ?: run {
            logger.w("legacy widget item position out of bounds: position=$position size=${data.size}")
            return null
        }

        val rv = RemoteViews(BuildConfig.APPLICATION_ID, R.layout.item_widget_agenda)
        ...
        val subheaderContent = if (event.isAllDay) {
            val allDayText = context.getString(R.string.event_all_day)
            if (event.subheaderContent.isNotEmpty()) {
                "$allDayText • ${event.subheaderContent.removePrefix(" • ")}"
            } else {
                allDayText
            }
        } else {
            event.subheaderContent
        }
        rv.setTextViewText(R.id.tv_event_subheader, subheaderContent)
        rv.setViewVisibility(R.id.tv_event_subheader, if (subheaderContent.isEmpty()) View.GONE else View.VISIBLE)
