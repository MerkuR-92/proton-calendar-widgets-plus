...
class UpdateCalendarUseCase @Inject constructor(
    ...
    suspend fun executeUpdateDisplayFromDb(userId: UserId, calendarId: String) : UseCase.Result {
        ...
        val dbMember = database.membersDao().selectCalendarMembers(calendarId).firstOrNull() ?: return UseCase.Result.Error("UpdateCalendarUseCase: executeUpdateFromDb DB Member was null")
    ...
    suspend fun executeUpdate(userId: UserId, calendarId: String, description: String? = null, name: String? = null, color: String? = null, display: Int? = null) : UseCase.Result {
        val dbMember = database.membersDao().selectCalendarMembers(calendarId).firstOrNull() ?: return UseCase.Result.Error("UpdateCalendarUseCase: executeUpdate DB Member was null")
