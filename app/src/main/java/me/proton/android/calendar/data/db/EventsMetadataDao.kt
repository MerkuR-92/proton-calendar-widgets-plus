...
abstract class EventsMetadataDao : BaseDao<EventEntityMetadata> {
    ...
    @Query("DELETE FROM events_metadata WHERE calendarId = :calendarId AND id IN (:eventIds)")
    abstract suspend fun deleteByEventIds(calendarId: String, eventIds: List<String>)
