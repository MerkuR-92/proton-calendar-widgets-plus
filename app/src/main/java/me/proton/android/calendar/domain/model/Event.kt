...
//        val dateTimeStamp: String
    ...
    fun generateOccurrencesUntilOrCount(timeZoneId: String, toDate: LocalDate?, occurrenceCount: Int?): List<Occurrence>? {
        ...
        val startZonedDateTime = ZonedDateTime.ofInstant(iCalEvent.dateStart.value.toInstant(),
            if (!hasTime && !isAllDay()) ZoneId.systemDefault()
        ...
        while (startIterator.hasNext()) {
            ...
            } else if (!hasTime && !isAllDay()) ZonedDateTime.ofInstant(startIteratorNextInstant, ZoneId.systemDefault()).withHour(startZonedDateTime.hour).withMinute(startZonedDateTime.minute)
            else ZonedDateTime.ofInstant(startIteratorNextInstant, ZoneId.of(timeZoneId))
            ...
            if (untilZonedDateTime != null) {
                if (this.isAllDay() && occurrenceStart.toLocalDate() == untilZonedDateTime.toLocalDate()) {
                    break
                } else if (occurrenceStart.isAfter(untilZonedDateTime)) {
                    break
                }
            }
        ...
        if (isRecurring()) {
            ...
            while (startIterator.hasNext() && counter <= occurrenceNumber) {
                ...
                if (counter == occurrenceNumber) {
                    ...
                    exceptionDates.values.add(ICalDate(nextValue, hasTime))
